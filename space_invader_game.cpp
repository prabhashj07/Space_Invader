...
int birdY = SCREEN_HEIGHT - 5;
int score = 0;

vector<array<int, 4>> bullets(20);
int bIndex = 0;
...
void drawBird(){
    for(int i=0; i<3; i++){
        for(int j=0; j<5; j++){
            gotoxy(j+birdPos, i+birdY); cout<<bird[i][j];
        }
    }
}
...
void play(){
    ...
    gotoxy(WIN_WIDTH + 2, 16);cout<<" W Key - Move Up";
    gotoxy(WIN_WIDTH + 2, 17);cout<<" S Key - Move Down";
    gotoxy(WIN_WIDTH + 2, 18);cout<<" Spacebar = Shoot";
